...
chmod 755 ${STARROCKS_HOME}/lib/be/starrocks_be
...
if [ ${RUN_DAEMON} -eq 1 ]; then
    nohup ${STARROCKS_HOME}/lib/be/starrocks_be --cn "$@" >> $LOG_DIR/cn.out 2>&1 </dev/null &
else
    ${STARROCKS_HOME}/lib/be/starrocks_be --cn "$@" >> $LOG_DIR/cn.out 2>&1 </dev/null
fi
